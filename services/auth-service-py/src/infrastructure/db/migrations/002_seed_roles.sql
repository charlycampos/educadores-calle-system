...
MERGE INTO SEC_ROL t
USING (SELECT 'MONITOR' AS nombre, 'Sectorista central. Supervisa calidad nacional y aprueba traslados externos.' AS descripcion FROM DUAL) s
ON (t.nombre = s.nombre)
WHEN NOT MATCHED THEN INSERT (nombre, descripcion) VALUES (s.nombre, s.descripcion);

MERGE INTO SEC_ROL t
USING (SELECT 'ESTADISTICO' AS nombre, 'Personal de estadistica. Descarga reportes y dashboard consolidado.' AS descripcion FROM DUAL) s
ON (t.nombre = s.nombre)
WHEN NOT MATCHED THEN INSERT (nombre, descripcion) VALUES (s.nombre, s.descripcion);
