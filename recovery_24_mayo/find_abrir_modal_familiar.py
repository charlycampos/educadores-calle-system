        ...
        if os.path.exists(transcript_path):
            with open(transcript_path, 'r', encoding='utf-8', errors='ignore') as f:
                for idx, line in enumerate(f):
                    if "abrirModalFamiliar" in line:
                        matches.append((folder, idx, line))

print(f"\nTotal matches: {len(matches)}")
for m in matches[:10]:
    # Parse the line to print a clean snippet of the content if available
    try:
        data = json.loads(m[2])
        content = data.get("content", "")
        if content:
            # Look for lines containing abrirModalFamiliar
            content_lines = content.splitlines()
            for c_idx, cl in enumerate(content_lines):
                if "abrirModalFamiliar" in cl:
