        ...
        if os.path.exists(transcript_path):
            with open(transcript_path, 'r', encoding='utf-8', errors='ignore') as f:
                for idx, line in enumerate(f):
                    if "nnaSemaforo[index] === 'error'" in line:
    ...
    try:
        ...
        if content:
            ...
            for c_idx, cl in enumerate(content_lines):
                if "nnaSemaforo[index] === 'error'" in cl:
                    print(f"\nFolder: {m[0]} | Transcript Line: {m[1]} | Content Line: {c_idx}")
                    for offset in range(-10, 80):
                        if 0 <= c_idx + offset < len(content_lines):
                            line_text = content_lines[c_idx + offset]
                            clean_text = line_text.encode('ascii', errors='replace').decode('ascii')
                            print(f"  {c_idx + offset + 1}: {clean_text}")
                    break
