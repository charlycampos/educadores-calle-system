...
try {
    ...
    // 1. UPDATE STATE
    // Target: serviciosBasicos: { agua: false, luz: false, desague: false },
    const oldStateToken = 'serviciosBasicos: { agua: false, luz: false, desague: false },';
    const newStateBlock = `serviciosBasicos: { agua: false, luz: false, desague: false, otros: false },
        viviendaSisfoh: '',
        duermeCama: '',
        duermeConQuien: '',
        duermeSoloAcompanado: '',
        higieneDomicilio: '',
        tiempoAlbergue: '',`;

    if (content.includes(oldStateToken)) {
        content = content.replace(oldStateToken, newStateBlock);
...
`;
    ...
    // Iterate reverse to handle shifts safely (though we break after one match)
    for (let i of indices) {
        const chunk = content.substring(i, i + 800);
        // Look for typical INPUT indicators: 'className="bg-white', 'onChange', 'value={formData'
        if ((chunk.includes('className="') || chunk.includes("className='")) && chunk.includes('onChange')) {
            // Identified Input Block
            const nextSection = content.indexOf('{/* VI.', i);
            if (nextSection !== -1) {
                const head = content.substring(0, i);
                const tail = content.substring(nextSection);
                content = head + newSectionV + '\n\n                    ' + tail;
                console.log('Section V replaced successfully.');
                break;
            }
        }
    }
