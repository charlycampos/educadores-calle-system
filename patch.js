...
const filePath = 'D:/Usuarios/ccampos/Documents/Python Scripts/Educadores_calle/educadores-calle-system/client/src/features/nna/NnaCreatePage.tsx';
let content = fs.readFileSync(filePath, 'utf-8');

const replacement =     const onSubmit = async (data: NnaFormData) => {
        setSubmitting(true);
        try {
            ...
            const legacyActividadesPerfil = data.actividadesCalle.map(act => {
                ...
                return {
                    ...
                    tiempoDetalle: act.tiempoUnidad === 'Detalle' ? act.tiempoValor : \\ \\,
                    jornada,
            ...
            const payloadToSave = {
                ...
                tiempoEnCalle: data.actividadesCalle.map(a => a.tiempoUnidad === "Detalle" ? a.tiempoValor : \\ \\).join(", ")
            };;

content = content.replace(/const onSubmit = async \(data: NnaFormData\) => \{[\s\S]*?const payloadToSave = \{[\s\S]*?\};/, replacement);
